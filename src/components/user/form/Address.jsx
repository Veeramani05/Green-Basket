...
class Address extends PureComponent {
      ...
      <div className="hole-div">
        <div className="address-div">
          ...
          {v["address"] && v["city"] && v["state"] ?
            <button type="button" className={`btn-style btn-font btn-bg ${this.state.addressIndex === i ? "bg-success" : ''}`} onClick={async () => await this.setState({ addressIndex: i })}> Deliver to this address {this.state.addressIndex === i ? <span class="color-success">✔</span> : ''}
            </button> : <span style={{ fontSize: "0.7rem", background: "red", padding: "0.1rem 01rem", borderRadius: "1rem", color: "#fff" }}>Please update your Address</span>}
        </div>
  ...
  updateAddress = async () => {
    ...
    try {
      ...
      const { address2, pincode2, city2, state2, secondaryContactNo, addrType } = data;
   ...
   */
  onSubmit = async () => {
    const { isCOD, isOTPverified, newAddress, otpfeild, isOTPsended, addressIndex } = this.state;
  ...
  delAddressChoose = async () => {
    ...
    if (isCOD && isOTPverified && !newAddress) return await this.paymentLoad(address);
          ...
          {({ formState }) => (
            <Container>
              <div className="well cartwell ">
                ...
                <Row className="adddetails" >
                  <Col md={12} className="contactformrowpadding">
                    ...
                    <Row>
                      <Col md={3} sm={12} >
                        <CustomRadio field="deliverytime" label="Delivery Time" name="delivery" options={this.state.deliverytime} checked={this.state.checked} validateOnBlur validate={e => this.validateProperty('deliverytime', e)} onChange={(e) => this.getDeliveryValue("delivery", e)} />
                      </Col>
                    </Row>
                    <Row>
                      {delivery === 'Preferred Delivery Time' &&
                        <Col md={6} sm={12} >
                          <DatetimePickerTrigger
                            ...
                            <Input field="preDeliveryTime" name="preDeliveryTime" autocomplete="off" label="Pre Delevery Time" validateOnBlur validate={e => this.validateProperty('preDeliveryTime', e)} forwardedRef={this.preDeliverDate} />
                          </DatetimePickerTrigger>
                        </Col>
                      }
                    </Row>
                    ...
                    <Row>
                      ...
                      <Col md={3} style={{ paddingTop: "0.4rem" }}>
                        {isCOD && !isOTPverified && otpfeild && !newAddress && <Button type="button" onClick={this.sendOtp} color={"warning"} className="mt-3 mr-2" size={"sm"}   >Resend OTP</Button>}
                        {isCOD && !isOTPverified && !newAddress ? <Fragment>
                    ...
                    <Row>
                      {!newAddress && <Col md={12} id="savedaddress" name="btn">
                        <p className="note">Note : <span> Please select the delivery address </span></p>
                    ...
                    </Row>
                    {
                      ...
                      <Fragment>
                        <Row>
                          <Col md={3} sm={12} >
                            <CustomRadio field="addrType" label="Address Type" name="addrType" options={addressTypeList} checked={this.state.checked} validateOnBlur validate={e => this.validateProperty('addrType', e)} onChange={(e) => this.getDeliveryValue("addrType", e)} />
                          </Col>
                        </Row>
                        ...
                        <Row>
                          ...
                          <Col style={{ textAlign: 'right' }}>
                            ...
                            <Button disabled={formState.invalid} color={"success"} size={"sm"}>Update & Deliver to this address</Button>
                          </Col>
                        </Row>
                      </Fragment>
              ...
              <div className="text-right">
                <Button variant="contained" type="button" color="warning" size={"sm"} className="mr-3" onClick={() => handleBack()} > Back </Button>
                <Button variant="contained" type="submit" color="primary" size={"sm"} className="ml-3" > Proceed to checkout </Button>
              </div>
            </Container>
