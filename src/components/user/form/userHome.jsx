...
const items = [
  ...
  { src: carouselimagemobile3, altText: 'Slide 2' },
  { src: carouselimagemobile4, altText: 'Slide 2' },
  { src: carouselimagemobile5, altText: 'Slide 2' },
  { src: carouselimagemobile6, altText: 'Slide 2' },
  { src: carouselimagemobile7, altText: 'Slide 2' },
  { src: carouselimagemobile8, altText: 'Slide 2' },
  { src: carouselimagemobile9, altText: 'Slide 2' },
  { src: carouselimagemobile10, altText: 'Slide 2' },
  { src: carouselimagemobile11, altText: 'Slide 2' },

];



const ArrowLeft = <div className='arrow-prev'><IosIcons.IoIosArrowBack /></div>
const ArrowRight = <div className='arrow-next'><IosIcons.IoIosArrowForward /> </div>
...
class Home extends PureComponent {
  ...
  addtoCart = async (data) => {
    const { productName, productID, productId, productUomId, categoryID, categoryId } = data;
  ...
  addtoWishlist = async (data) => {
    const { productID, productId, productUomId, productName } = data;
    let postData = {
      "userId": this.state.uid,
      "productId": productID || productId,
      "quantity": 1,
  ...
  frameLoad1 = () => {
    ...
    return _.map(seacenBasketList, (c, i) =>
      <Card prodname={c.productName} fixprice={c.sellingPrice} offerprice={c.mrp} prodcount={c.productUomId + c.productUom} imgsource={c.image} key={`bestSellingprod${i}`}>
        <div class="">
          <div class="ribbon"><span> {Math.round(((c["mrp"] - c["sellingPrice"]) * 100) / c["mrp"])} % OFF</span></div>
        </div>
        <div className="row text-center" >
  ...
  render() {
    ...
    var background = { backgroundSize: 'cover', position: "relative", zIndex: 2, width: "auto", maxWidth: "85%" };
