...
class Payment extends PureComponent {
  ...
  paymentLoad = () => {
    ...
    try {
      return <iframe title="Payment" id="paymentFrame" ref={"paymentFrame"} className="payment-frame" src={`${paymentGatwayURI}/about?amt=${netAmount}&email=${email}&mobileno=${delPhone}`} onLoad={this.iframeLoad}>
      </iframe>
    } catch (err) {
  ...
  placeOrder = async (payload) => {
    try {
      ...
      if (statusCode) {
        if (transactionStatus === 'S') await this.addNotification("Order was Placed Successfully")
        if (transactionStatus !== 'S') await this.addNotification("Order was Canceled BY User.", 'warning')
        _.assign(tansData, data[0])
        return this.redirectTo(tansData, "forward");
      }
      return this.addNotification("Something went wrong please try again after some time", "error");
    } catch (err) {
    }
  }
    ...
    return (
      <Fragment>
        ...
        <div className="text-right">
          <Button onClick={() => handleBack()} color="warning" size={"sm"} > Back</Button> 
        </div>
      </Fragment>
    )
  }
